package sec1;
//정적 내부 클래스인 Inner 클래스는 외부든 내부든 정적 요소만 허용함
class Outer2{
	private int num = 10;
	private static int sNum=20; //static 정적
	...
	static class Inner { // 정적 내부 클래스
		...
		void inMethod() {
			System.out.println("내부에서 클래스의 인스턴스 변수 : inNum = "+inNum);
			System.out.println("내부에서 클래스의 정적 멤버 변수 : sInNum = "+sInNum);
//			System.out.println("내부에서 외부 클래스의 인스턴스 멤버 변수 : num = "+num); 
//			System.out.println("외부 클래스의 인스턴스 멤버 변수 : num = "+num); //공유하는 데이터가 아니라서 에러 인스턴스가 없어서?
			System.out.println("내부에서 외부 클래스의 정적 멤버 변수 : sNum = "+sNum);
		}
	}
	static void outMethod() { //외부 클래스의 정적 메서드
//		System.out.println("외부에서 클래스의 인스턴스 변수 : inNum = "+Inner.inNum); 내부 클래스가 static 
//		System.out.println("외부에서 클래스의 인스턴스 변수 : inNum = "+inNum);
		System.out.println("외부에서 내부 클래스의 정적 멤버 변수 : sInNum = "+Inner.sInNum);
//		System.out.println("외부에서 외부 클래스의 인스턴스 멤버 변수 : num = "+num); 
